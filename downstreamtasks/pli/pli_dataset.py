...
class Protein_search(Dataset):
    ...
    def __getitem__(self, index):
        ...
        ligand_x = torch.tensor(data['ligand_x'], dtype=torch.float)
        ligand_edge_index = torch.tensor(data['ligand_edge_index'], dtype=torch.long)
        edge_attr = torch.tensor(data['edge_attr'], dtype=torch.float)
        score = torch.tensor(data['y'], dtype=torch.float)
        ligand_graph = Data(x=ligand_x, edge_attr=edge_attr, edge_index=ligand_edge_index, y=score.clone().detach())
        ...
        return  protein_graph,xyz, curvature, dists, atom_type_sel,ligand_graph
