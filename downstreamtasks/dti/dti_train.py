...
from downstreamtasks.dti.models.dti_protein_multimodal import DTIPredictor
from utils import data_transforms
from downstreamtasks.dti.dti_dataset import Protein_search
...
def collate_fn(batch):
    protein_graphs, xyzs, curvatures, dists, atom_type_sels, drug_graphs, labels = zip(*batch)
...
def main():
    parser = argparse.ArgumentParser(description='Drug-Target Interaction Prediction Training.')
    ...
    train_set = Protein_search(data_root=cfg.data.train_dir, K=cfg.data.K, sample_num=cfg.data.sample_num, seed=cfg.seed)
    val_set = Protein_search(data_root=cfg.data.val_dir, K=cfg.data.K, sample_num=cfg.data.sample_num, seed=cfg.seed)
    test_set = Protein_search(data_root=cfg.data.test_dir, K=cfg.data.K, sample_num=cfg.data.sample_num, seed=cfg.seed)
    ...
    model = DTIPredictor(config=config, device=device).to(device)
