...
PLiSAGE 是一个用于蛋白质结构表示学习的深度学习框架。它通过一个新颖的多模态学习策略，同时从蛋白质的**原子级结构图（Structure Graph）**和**三维表面几何（Surface Geometry）**中学习特征，旨在生成更全面、更鲁棒的蛋白质表示，以用于下游生物学任务，如蛋白质-配体相互作用（PLI）和药物-靶点相互作用（DTI）的预测。
