...
class MaskTransformer(nn.Module):
    def __init__(self, config, in_dim=16, out_dim=18, **kwargs):
        super().__init__()
        self.config = config

        self.in_dim = config.in_channels
        self.out_dim = config.emb_dims
        self.trans_dim = self.out_dim*config.transformer_config.dimscale
    ...
    def forward(self, neighborhood, center, noaug = False):
        ...
        group_input_tokens = self.encoder(neighborhood[:,:,:,3:])  #  B G C
...
class Point_MAE(nn.Module):
    def __init__(self, config, in_dim=16, out_dim=8):
        ...
        self.MAE_encoder = MaskTransformer(config, self.in_dim, self.out_dim)
    ...
    def forward(self, pts, curvature, dist, atom_type, vis = False, **kwargs):
        ...
        # 假设 x_vis 是你想要从中提取特征的张量
        concat_f = torch.cat([x_vis[:, 0], x_vis[:, 1:].max(1)[0]], dim=-1)
        global_future= self.fc(concat_f)
